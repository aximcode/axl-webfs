...
- **Foreground (default)** — `axl-webfs.efi serve` loads the
  embedded driver via `axl_service_start_embedded`, registers an
  ESC key handler on the default AxlLoop, and supervises via
  `axl_service_supervise`, which blocks on the loop until ESC
  quits it and then calls `axl_service_stop` to unload the driver.
  The HTTP server itself runs inside the driver image.

- **Detached** — `axl-webfs.efi serve --detach` loads the same
  driver via `axl_service_start_embedded` and returns to the
  shell without supervising. The driver image (`axl-webfs-serve-dxe.efi`) is embedded
  into `axl-webfs.efi` via `axl-cc --embed` (a `.incbin` sidecar
  generated by the SDK build wrapper) so the toolkit ships as a
  single binary. The driver's
