/** @file
  axl-webfs-dxe -- Driver entry point, URL parsing, protocol install.
...
/// Vendor GUID for the axl-webfs device path node.
...
static WebFsPrivate *mPrivate = NULL;
...
static WebFsDevicePath mDevicePathTemplate = {
    {
...
)
{
    ...
    if (url_utf8 == NULL) {
        axl_printf("axl-webfs-dxe: No URL in load options\n");
    ...
    axl_free(url_utf8);
    if (ret != 0) {
        axl_printf("axl-webfs-dxe: Invalid URL\n");
        axl_free(priv);
        return EFI_INVALID_PARAMETER;
    }
    ...
    axl_printf("axl-webfs-dxe: Connecting to %d.%d.%d.%d:%d%s\n",
               priv->server_addr[0], priv->server_addr[1],
               priv->server_addr[2], priv->server_addr[3],
               priv->server_port, priv->base_path);
    ...
    if (net_ret != 0) {
        axl_printf("axl-webfs-dxe: Network init failed\n");
        axl_free(priv);
    ...
    if (priv->http_client == NULL) {
        axl_printf("axl-webfs-dxe: Failed to create HTTP client\n");
        network_cleanup();
        axl_free(priv);
    ...
    if (info_ret != 0 || info_resp == NULL || info_resp->status_code != 200) {
        axl_printf("axl-webfs-dxe: Server validation failed\n");
    ...
    axl_memcpy(dev_path, &mDevicePathTemplate, sizeof(WebFsDevicePath));
    ...
    if (axl_service_register_multiple(&priv->fs_handle,
            ...
            NULL) != 0) {
        axl_printf("axl-webfs-dxe: Protocol install failed\n");
        axl_free(dev_path);
        axl_http_client_free(priv->http_client);
        network_cleanup();
        axl_free(priv);
...
)
{
    ...
    axl_printf("axl-webfs-dxe: Unmounted\n");
    return EFI_SUCCESS;
}
